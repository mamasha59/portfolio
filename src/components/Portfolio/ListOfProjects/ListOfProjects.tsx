  ...
  return (
    <aside className="flex justify-start py-6 flex-col overflow-y-auto sl:justify-center sl:px-3 sl:max-w-[40%]">
        ...
        <div key={index} className="flex flex-col p-4 rounded-2xl sm:p-0">
            <h2 onClick={() => setIndex(index,i.title)} className={`${state.isDark ? 'text-orange' : 'text-lightModeT'} cursor-pointer ${currentIndex === index ? 'opacity-100 underline' : 'opacity-50'} sm:text-base`}>{i.title}</h2>
